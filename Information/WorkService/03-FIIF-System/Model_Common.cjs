...
module.exports.comm_get_status = async function (req, res) {
    try {
      const get_status = `SELECT "GC".safety_fiif_001_comm_get_status()`;
      const client = await ConnectPG_DB();
      const result_get_status = await client.query(get_status);
      await DisconnectPG_DB(client);
      const jsonData = result_get_status.rows.map((row) => {
        const rawData = row.safety_fiif_001_comm_get_status.replace(/^\((.*)\)$/, "$1");
