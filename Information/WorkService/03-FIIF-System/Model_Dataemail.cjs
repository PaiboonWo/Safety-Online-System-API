...
module.exports.Get_Issue_Approve_Data_Email = async function (req, res) {
  ...
  try {
    ...
    query += `SELECT * from "GC".safety_fiif_006_trans_get_issue_approve_send_email('[${json_convertdata}]')`;
    const result = await client.query(query);
    const filteredResult = result.rows.map((row) => row.response);
    console.log("safety_fiif_006_trans_get_issue_approve_send_email", filteredResult);
